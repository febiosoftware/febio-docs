...
**Type string:** `"sliding-facet-on-facet"`
...
| `gaptol` | gap tolerance | 0 | $\in \mathbb{R}$ | L |
| `minaug` | min. augmentations | 0 | $\in \mathbb{Z}$ |  |
| `maxaug` | max. augmentations | 10 | $\in \mathbb{Z}$ |  |
| `smooth_aug` | Smooth augmentations | false | $\{0, 1\}$ |  |
| `search_tol` | search_tol | 0.01 | $\in \mathbb{R}$ |  |
| `search_radius` | search_radius | 0 | $\in \mathbb{R}$ | L |
| `seg_up` | max. segment updates | 0 | $\in \mathbb{Z}$ |  |
| `node_reloc` | node relocation | false | $\{0, 1\}$ |  |
| `knmult` | Stiffness scale factor | 1 | $\in \mathbb{R}$ |  |
